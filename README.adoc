...
The following example demonstrates using optional parameters including `workspace-dir`, `cli-params`, and `build-directory`:

[source, yaml,role="default-expanded"]
----
name: snyk-sca-scan-with-options
kind: workflow
apiVersion: automation.cloudbees.io/v1alpha1
...
jobs:
  snyk-sca-scan:
    steps:
      ...
      - name: Snyk SCA Scan With Optional Parameters
        uses: cloudbees-io/snyk-sca-plugin-private@v1
        with:
          snyk-token: ${{ secrets.SNYK_TOKEN }}
          org-id: ${{ vars.SNYK_ORG_ID }}
          ref: main
          # Specify custom workspace directory to store the code
          workspace-dir: /custom/workspace/dir
          # Pass additional CLI parameters to Snyk
          cli-params: --all-projects --json
          # Specify the directory containing build files
          build-directory: app
